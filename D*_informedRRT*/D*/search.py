    ...
    def process_state(self):
        ...
        if(k_old < s.h):
            for node in neighbors:
                if node.h <= k_old and s.h > node.h + self.cost(node,s):
                    s.parent = node
                    s.h = node.h + self.cost(node,s)
        # If node k is the same as h (LOWER)
        if(k_old == s.h):
            for node in neighbors:
                if node.tag=="NEW" or (node.parent==s and node.h!=s.h+self.cost(s,node)) or (node.parent!=s and node.h>s.h+self.cost(s,node)):
                    node.parent = s
                    self.insert(node,s.h+self.cost(s,node))
        # Else node k is smaller than h (RASIE)
        else:
            for node in neighbors:
                if node.tag=="NEW" or (node.parent==s and node.h!=s.h+self.cost(s,node)):
                    node.parent = s
                    self.insert(node,s.h+self.cost(s,node))
                else:
                    if node.parent!=s and node.h>s.h+self.cost(s,node):
                        self.insert(s,s.h)
                    else:
                        if node.parent!=s and s.h>node.h+self.cost(node,s) and node.tag=="CLOSED" and node.h>k_old:
    ...
    def prepare_repair(self, node):
        ...
        for neighbor in neighbors:
            if neighbor.is_dy_obs==True and neighbor.is_obs==False:
                ...
                node_neighbors = self.get_neighbors(neighbor)
                for node_node in node_neighbors:
                    self.modify_cost(node,node_node)
        #### TODO END ####
