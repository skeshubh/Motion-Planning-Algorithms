...
class RRT:
    ...
    def get_new_point_in_ellipsoid(self, goal_bias, c_best):
        ...
        # Generate a random point in an ellipsoid
            ...
            c_min = self.dis(self.goal, self.start)
            ...
            x_center = np.array([[x_center_row],[x_center_col],[0.0]])
            # Compute rotation matrix from elipse to world frame - C
            a1 = np.array([[(self.goal.row - self.start.row)/c_min],[(self.goal.col - self.start.col)/c_min],[0.0]])
            e1 = np.array([[1.0],[0.0],[0.0]])
            M = a1 @ e1.T
            U, sigma, Vt = np.linalg.svd(M,True,True)
            C = U @ np.diag([1.0, 1.0, np.linalg.det(U) * np.linalg.det(Vt.T)]) @ Vt
            # Compute diagonal matrix - L
            r = [c_best/2.0, np.sqrt(c_best**2 - c_min**2)/2.0, np.sqrt(c_best**2 - c_min**2)/2.0]
            L = np.diag(r)
            # Cast a sample from a unit ball - x_ball
            
            u = np.random.normal(0,1,(1,3))
            norm = np.linalg.norm(u,axis=-1,keepdims=True)
            u = u/norm
            x_ball = np.zeros((3,1))
            x_ball[0:2,:] = np.atleast_2d(u[0,:2]).T
            x_rand = np.dot(C,np.dot(L,x_ball)) + np.atleast_2d(x_center).T
            ...
            while True:
                ...
                while True:
                    ...
                    if(x**2 + y**2 < 1):
                        break
                x_ball = np.array([[x],[y],[0.0]])
                # Map ball sample to the ellipsoid - x_rand
                x_rand = np.dot(np.dot(C,L), x_ball) + x_center
                if x_rand[0]>=0 and x_rand[0]<len(self.map_array) and x_rand[1]>=0 and x_rand[1]<len(self.map_array[0]):
                    break
            '''
            '''
            x,y = 0,0
